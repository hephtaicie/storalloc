...
class Request (object):
    ...
    def __init__ (self, capacity, duration):
        ...
        self._start_time = dt.datetime.now(dt.timezone.utc) + dt.timedelta(minutes=np.random.randint(180))
    ...
    def request_string (self):
        return ("["+str(self._capacity)+" GB, "+str(self._duration)+" m, "+str(self._start_time)+", "+str(self._end_time)+"]") 
...
def parse_args ():
    ...
    parser.add_argument ('-b', '--boxes', type=int, help="Number of NVMeoF boxes (default: "+str(n_nodes)+")")
    parser.add_argument ('-d', '--disks', type=int, help="Number of NVMe diskes per box (default: "+str(n_disks_per_node)+")")
    parser.add_argument ('-c', '--capacity', type=int, help="Capacity per disk in GB (default: "+str(disk_capacity)+")")
    parser.add_argument ('-r', '--requests', type=int, help="Number of requests to simulate (default: "+str(n_requests)+")")
    ...
    if args.boxes: n_nodes = args.boxes
    if args.disks: n_disks_per_node = args.disks
    if args.capacity: disk_capacity = args.capacity
    if args.requests: n_requests = args.requests
...
def main (argv):
    ...
    print ("   - "+str(n_disks_per_node)+" NVMe disks of "+str(disk_capacity)+" GB each per box")
    ...
    for new_r in requests:
        ...
        for b in range (0, len(storage_resources)):
            ...
            if target_disk is None:
                logging.debug ("Unable to allocate the current request on node "+str(current_node.get_idx()))

        # If a disk on a node has been found, we allocate the request
        if target_disk is not None:
            logging.debug ("Add request "+new_r.request_string()+" on node "+str(target_node.get_idx())+", disk "+str(target_disk.get_idx()))
