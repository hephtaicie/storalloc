...
def main (argv):
    ...
    workload_traces = subset_traces[["COBALT_JOBID",
                                     "RUN_DATE_ID",
                                     "USER_ID",
                                     "EXE_NAME_GENID",
                                     "QUEUED_TIMESTAMP",
    ...
    workload_traces.loc[:, ('QUEUED_TIMESTAMP')] = workload_traces.QUEUED_TIMESTAMP.apply(str_to_timestamp)
    workload_traces.loc[:, ('START_TIMESTAMP')] = workload_traces.START_TIMESTAMP.apply(str_to_timestamp)
    workload_traces.loc[:, ('END_TIMESTAMP')] = workload_traces.END_TIMESTAMP.apply(str_to_timestamp)
    ...
    io_data = current_traces [['COBALT_JOBID',
                               'NPROCS',
                               'QUEUED_TIMESTAMP',
                               'START_TIMESTAMP',
                               'END_TIMESTAMP',
    ...
    # Add custom metrics
    io_data.loc[:, ('RatioWriteTimeRunTime')] = (io_data.TOTAL_MPIIO_F_WRITE_TIME/io_data.NPROCS)/io_data.RUN_TIME
    ...
    io_jobs = io_data.loc[(io_data['RatioWriteTimeRunTime']>=0.1) |
    ...
    for index, row in io_jobs.iterrows():
        d_io_jobs['jobs'].append({'id': int(row['COBALT_JOBID']),
    ...
    with open("IOJobs"+calendar.month_abbr[month]+".yml", 'w') as yamlfile:
        ...
        print(calendar.month_name[month]+" IO intensive jobs extracted successfully to IOJobs"+calendar.month_abbr[month]+".yml")
