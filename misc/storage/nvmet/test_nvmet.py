...
NVMET_TEST_DEVICES = os.getenv("NVMET_TEST_DEVICES", "/dev/ram0,/dev/ram1").split(",")
...
def test_devices_present():
    return (
        len(
            [
                x
                for x in NVMET_TEST_DEVICES
                if os.path.exists(x) and (stat.S_ISBLK(os.stat(x).st_mode) or os.path.isfile(x))
            ]
        )
        >= 2
    )
...
class TestNvmet(unittest.TestCase):
    def test_subsystem(self):
        ...
        # duplicate
        self.assertRaises(nvme.CFSError, nvme.Subsystem, nqn="testnqn1", mode="create")
        self.assertEqual(len(list(root.subsystems)), 3)
    ...
    def test_namespace_attrs(self):
        ...
        self.assertRaises(nvme.CFSError, n.set_attr, "device", "path", NVMET_TEST_DEVICES[1])
        self.assertRaises(
            nvme.CFSError, n.set_attr, "device", "nguid", "15f7767b-50e7-4441-949c-75b99153dea7"
        )
    ...
    def test_loop_port(self):
        ...
        self.assertRaises(nvme.CFSError, p.set_attr, "addr", "traddr", "10.0.0.1")
        self.assertRaises(nvme.CFSError, p.set_attr, "addr", "treq", "required")
    ...
    def test_host(self):
        ...
        # duplicate
        self.assertRaises(nvme.CFSError, nvme.Host, "foo", mode="create")
        self.assertEqual(len(list(root.hosts)), 2)
    ...
    def test_referral(self):
        ...
        # duplicate
        self.assertRaises(nvme.CFSError, nvme.Referral, p, name="2", mode="create")
        self.assertEqual(len(list(p.referrals)), 2)
        ...
        # non-existant lookup
        self.assertRaises(nvme.CFSError, nvme.Referral, p, name="foo", mode="lookup")
        ...
        self.assertRaises(nvme.CFSError, r.set_attr, "addr", "traddr", "10.0.0.1")
        self.assertRaises(nvme.CFSError, r.set_attr, "addr", "treq", "required")
    ...
    def test_invalid_input(self):
        ...
        for l in [257, 512, 1024, 2048]:
            toolong = "".join(random.choice(string.ascii_lowercase) for i in range(l))
            self.assertRaises(nvme.CFSError, nvme.Subsystem, nqn=toolong, mode="create")

        discover_nqn = "nqn.2014-08.org.nvmexpress.discovery"
        self.assertRaises(nvme.CFSError, nvme.Subsystem, nqn=discover_nqn, mode="create")

        self.assertRaises(nvme.CFSError, nvme.Port, portid=1 << 17, mode="create")
    ...
    def test_save_restore(self):
        ...
        self.assertRaises(nvme.CFSError, root.restore_from_file, "test.json", False)
