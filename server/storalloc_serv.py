...
def parse_args ():
    ...
    parser.add_argument ('-r', '--reset', help="Reset the existing storage configurations", action='store_true')
    parser.add_argument ('--simulate', help="Receive requests only. No actual storage allocation", action='store_true')
...
def reset_resources (storage_resources):
    ...
    while confirm not in ["y", "n"]:
        confirm = input("Are you sure you want to reset the existing NVMeoF configuration"
                        +", including disk partitions [Y/N]? ").lower()
    if confirm == "y":
