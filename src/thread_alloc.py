...
class ThreadAlloc(threading.Thread, metaclass=ThreadAlloc_meta):
    ...
    def checkDeallocation(self):
        ...
        if(ThreadAlloc.time >= self._allocTimestamp + datetime.timedelta(minutes=self._allocDuration)):
            print("Thread "+str(self._allocID)+" resources have to be deallocated, releasing the lock -> back to run()")
            if not self._lockAllocation.locked() : self._lockAllocation.acquire()
    ...
    def run(self):
        ...
        print("Thread "+str(self._allocID)+" will deallocate the resources when the allocation duration is over")

        self._lockAllocation.acquire()
        print("Thread "+ str(self._allocID)+" resources will be released, submitting request to the client ...")
        ThreadAlloc.instances.remove(self)
        deallocationClientCall = "./client.py -c config/client/config.yml -s "+str(self._allocCapacity)+" -t "+str(self._allocDuration)+" -v -d "+str(self._allocID)+" -timestamp \""+str(ThreadAlloc.time)+"\""
