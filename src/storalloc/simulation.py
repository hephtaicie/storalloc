...
class Simulation:
    ...
    def update_disk(self, server_id, node_id, disk_id, allocation):
        ...
        # free some disk space
            ...
            self.log.error(
                f"[SIM] Attempting to allocate/deallocate 0GB on {server_id}:{node_id}:{disk_id}"
            )
    ...
    def allocate(self, request: StorageRequest):
        ...
        self.update_disk(request.server_id, request.node_id, request.disk_id, -storage)
        self.log.debug(f"[SIM] {request.job_id} deallocated from disk")
        self.stats["concurrent_allocations"] -= 1
    ...
    def pre_sim_summary(self):
        ...
        print(f"## Number of events in simulation: {self.stats['events_nb']}")
    ...
    def simulation(self):
        ...
        self.log.info(f"Maximum number of concurrent allocations: {self.stats['max_ca']}")
        self.log.info(f"Total GB allocated: {self.stats['total_gb_alloc']}")
        self.log.info(f"Total GB deallocated: {self.stats['total_gb_dealloc']}")
