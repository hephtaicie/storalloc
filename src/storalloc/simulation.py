...
class Simulation:
    ...
    def update_disk(self, server_id, node_id, disk_id, allocation):
        ...
        # free some disk space
            ...
            self.log.error(
                f"[SIM ERROR] Attempting to allocate/deallocate 0GB on {server_id}:{node_id}:{disk_id}"
            )
    ...
    def process_registration(self, identity: bytes, node_data: dict):
        ...
        for data in node_data:
            ...
            for disk in node.disks:
                ...
                self.log.info(
                    f"[PRE-SIM] New simpy.Container for disk {disk.uid}, with capacity {disk.capacity}"
                )
                setattr(
