...
class CFSNode(object):
    ...
    def _create_in_cfs(self, mode):
        ...
        if self.exists and mode == "create":
            raise CFSError("This %s already exists in configFS" % self.__class__.__name__)
        elif not self.exists and mode == "lookup":
            raise CFSNotFound("No such %s in configfs: %s" % (self.__class__.__name__, self.path))

        if not self.exists:
            try:
                os.mkdir(self.path)
            except:
                raise CFSError("Could not create %s in configFS" % self.__class__.__name__)
        self.get_enable()
    ...
    def _check_self(self):
        if not self.exists:
            raise CFSNotFound("This %s does not exist in configFS" % self.__class__.__name__)
    ...
    def list_attrs(self, group, writable=None):
        ...
        names = [
            os.path.basename(name).split("_", 1)[1]
            for name in glob("%s/%s_*" % (self._path, group))
            if os.path.isfile(name)
        ]

        if writable is True:
            names = [name for name in names if self._attr_is_writable(group, name)]
        elif writable is False:
            names = [name for name in names if not self._attr_is_writable(group, name)]
    ...
    def set_attr(self, group, attribute, value):
        ...
        if self._enable:
            raise CFSError("Cannot set attribute while %s is enabled" % self.__class__.__name__)
    ...
    def set_enable(self, value):
        ...
        try:
            ...
            raise CFSError("Cannot enable %s: %s (%s)" % (self.path, e, value))
        self._enable = value
...
class Root(CFSNode):
    def __init__(self):
        ...
        if not os.path.isdir(self.configfs_dir):
            raise CFSError("%s does not exist.  Giving up." % self.configfs_dir)
    ...
    subsystems = property(_list_subsystems, doc="Get the list of Subsystems.")
    ...
    ports = property(_list_ports, doc="Get the list of Ports.")
    ...
    hosts = property(_list_hosts, doc="Get the list of Hosts.")
    ...
    def restore_from_file(self, savefile=None, clear_existing=True, abort_on_error=False):
        """
        ...
        with open(savefile, "r") as f:
            ...
            return self.restore(
                config, clear_existing=clear_existing, abort_on_error=abort_on_error
            )
...
class Subsystem(CFSNode):
    ...
    namespaces = property(_list_namespaces, doc="Get the list of Namespaces for the Subsystem.")

    def _list_allowed_hosts(self):
        return [os.path.basename(name) for name in os.listdir("%s/allowed_hosts/" % self._path)]

    allowed_hosts = property(
        _list_allowed_hosts, doc="Get the list of Allowed Hosts for the Subsystem."
    )
    ...
    def add_allowed_host(self, nqn):
        ...
        try:
            os.symlink(
                "%s/hosts/%s" % (self.configfs_dir, nqn), "%s/allowed_hosts/%s" % (self._path, nqn)
            )
        except Exception as e:
...
class Namespace(CFSNode):
    ...
    subsystem = property(_get_subsystem, doc="Get the parent Subsystem object.")
...
class Port(CFSNode):
    ...
    def _list_subsystems(self):
        return [os.path.basename(name) for name in os.listdir("%s/subsystems/" % self._path)]

    subsystems = property(_list_subsystems, doc="Get the list of Subsystem for this Port.")
    ...
    def add_subsystem(self, nqn):
        ...
        try:
            os.symlink(
                "%s/subsystems/%s" % (self.configfs_dir, nqn),
    ...
    referrals = property(_list_referrals, doc="Get the list of Referrals for this Port.")
    ...
    ana_groups = property(_list_ana_groups, doc="Get the list of ANA Groups for this Port.")
