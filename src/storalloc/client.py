...
class Client:
    ...
    def run(
        ...
        self.log.info(f"New request generated by {self.uid} : {request}")
        ...
        while True:
            ...
            if message.category == MsgCat.NOTIFICATION:
                ...
                # Remember how many notifications come back, in case the request has been splitted
                ...
                if request.divided > 1:
                    recv += 1
                    parts -= 1
                    self.log.info(f"Request answered (part {recv}/{request.divided}): {request}")

                if parts == 0 or recv == request.divided:
                    self.log.info(
                        f"Received {recv} / {request.divided} - {parts} "
                        + f"missing parts compared to what was expected. Client {self.uid} exiting now."
                    )
