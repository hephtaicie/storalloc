    ...
    def create_disk_allocation(self, node_ipv4, device, capacity):
        ...
        if space is None:
            ...
            geometry = parted.Geometry(
                start=free_space[space].start, length=sectors_required, device=self._dev_nvme
            )
            filesystem = parted.FileSystem(type="xfs", geometry=geometry)

            self._partition = parted.Partition(
                disk=self._disk_nvme, type=parted.PARTITION_NORMAL, fs=filesystem, geometry=geometry
            )

            self._disk_nvme.addPartition(
                self._partition, constraint=self._dev_nvme.optimalAlignedConstraint
            )
            self._disk_nvme.commit()

            part_dev = "/dev/" + str(self._partition.getDeviceNodeName())
            logging.debug(
                "New partition " + part_dev + " created from " + str(free_space[space].start)
            )
        ...
        self._namespace.set_attr(
            "device", "path", "/dev/" + str(self._partition.getDeviceNodeName())
        )
        self._namespace.set_enable("1")
        logging.debug(
            "New NVMe namespace created: " + str(self._namespace._get_nsid()) + ", path=" + part_dev
        )
